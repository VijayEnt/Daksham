...
public class item extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("ssave")!= null){
            ...
            String itid = request.getParameter("dname");
            String iname = request.getParameter("gname");
            PreparedStatement ptst = connection.prepareStatement("Update mstitem set isActive = 'Y',actiondate = now(),actionuserID = 1 where itemid = '"+itid+"'");
            ptst.executeUpdate();
            out.println("<script type=\"text/javascript\">");            
            out.println("alert('"+iname+" Activated!');");
            out.println("location='itemlookup.jsp';");
            out.println("</script>");                
            }
            catch(Exception ex){
